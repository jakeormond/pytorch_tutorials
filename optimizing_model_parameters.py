...
# Now that we have a model and data it’s time to train, validate and 
# test our model by optimizing its parameters on our data. Training a 
# model is an iterative process; in each iteration the model makes a 
# guess about the output, calculates the error in its guess (loss), 
# collects the derivatives of the error with respect to its parameters 
# (as we saw in the previous section), and optimizes these parameters 
...
# training loop. Jump ahead to see the Full Implementation of the optimization loop.

# Loss Function
# When presented with some training data, our untrained network is likely not to 
# give the correct answer. Loss function measures the degree of dissimilarity of 
# obtained result to the target value, and it is the loss function that we want 
...
# Gradients by default add up; to prevent double-counting, we explicitly zero them 
# at each iteration.
