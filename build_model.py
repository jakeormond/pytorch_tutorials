...
# your own neural network. Every module in PyTorch subclasses the nn.Module. 
...
# network layers in __init__. Every nn.Module subclass implements the operations on 
# input data in the forward method.
...
# Calling the model on the input returns a 2-dimensional tensor with dim=0 corresponding 
# to each output of 10 raw predicted values for each class, and dim=1 corresponding to 
# the individual values of each output. We get the prediction probabilities by passing 
# it through an instance of the nn.Softmax module.
...
# Model Layers
# Let’s break down the layers in the FashionMNIST model. To illustrate it, we will take 
# a sample minibatch of 3 images of size 28x28 and see what happens to it as we pass it 
# through the network.
...
# modules in the same order as defined. You can use sequential containers to put together 
...
# representing the model’s predicted probabilities for each class. dim parameter indicates 
...
# biases that are optimized during training. Subclassing nn.Module automatically tracks 
# all fields defined inside your model object, and makes all parameters accessible using 
# your model’s parameters() or named_parameters() methods.
