...
# is back propagation. In this algorithm, parameters (model weights) 
# are adjusted according to the gradient of the loss function with 
# respect to the given parameter.

# To compute those gradients, PyTorch has a built-in differentiation engine 
# called torch.autograd. It supports automatic computation of gradient 
# for any computational graph.

# Consider the simplest one-layer neural network, with input x, parameters 
# w and b, and some loss function. It can be defined in PyTorch in the following manner:
...
# This code defines the following computational graph: https://pytorch.org/tutorials/_images/comp-graph.png
...
# ∂loss/∂w and ∂loss/∂b under some fixed values of x and y. To compute those derivatives,
...
# graph, which have requires_grad property set to True. For all other nodes in our 
# graph, gradients will not be available.
...
# to apply it to some input data, i.e. we only want to do forward computations through 
# the network. We can stop tracking computations by surrounding our computation code 
...
# from scratch; after each .backward() call, autograd starts populating a new graph. 
# This is exactly what allows you to use control flow statements in your model; you can 
# change the shape, size and operations at every iteration if needed.

# Optional Reading: Tensor Gradients and Jacobian Products
# In many cases, we have a scalar loss function, and we need to compute the gradient 
# with respect to some parameters. However, there are cases when the output function is 
# an arbitrary tensor. In this case, PyTorch allows you to compute so-called Jacobian product, 
# and not the actual gradient.

# For a vector function... (formula didn't render, see link 
# https://pytorch.org/tutorials/beginner/basics/autogradqs_tutorial.html) 
# ...is given by Jacobian matrix: 
# (formula didn't render, see link
# Instead of computing the Jacobian matrix itself, PyTorch allows you to compute Jacobian Product
# ... for a given input vector v = .... This is achieved by calling backward with v as an argument.
# The size of v should be the same as the size of the original tensor, with respect to which we 
# want to compute the product:
...
# with the same argument, the value of the gradient is different. 
# This happens because when doing backward propagation, PyTorch 
# accumulates the gradients, i.e. the value of computed gradients 
# is added to the grad property of all leaf nodes of computational graph. 
# If you want to compute the proper gradients, you need to zero out the
# grad property before. In real-life training an optimizer helps us to do this.
